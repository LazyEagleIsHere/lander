...
# --- Your existing colors, screen setup, font, etc. ---
...
# --- Helper to get spaceship polygon (rectangle polygon) ---
def get_spaceship_polygon(pos):
    x, y = pos
    w, h = 10, 10  # spaceship size consistent with your draw.rect size
    return [Vector2(x, y), Vector2(x + w, y), Vector2(x + w, y + h), Vector2(x, y + h)]

# --- Main game loop with polygon collision detection ---
def game():
    global spaceship_pos, gravity, a, v1, v2, fuel, platform_pos, platform_ground1, platform_ground2
    ...
    platform_pos = [randint(200, width - 200), randint(height // 2 + 100, height - 10)]
    platform_ground1 = randint(200, 300)  # reasonable platform left width
    platform_ground2 = randint(200, 300)  # reasonable platform right width
    ...
    while gaming:
        ...
        # Define ground polygon based on platform position and widths
        ground_polygon = [
            Vector2(platform_pos[0], platform_pos[1] + 2),
        ...
        # Get spaceship polygon points
        spaceship_poly = get_spaceship_polygon(spaceship_pos)
        
        # Draw ground polygon
        pygame.draw.polygon(screen, gray, [(p.x, p.y) for p in ground_polygon])
        # Draw platform rectangle on top
        ...
        # Check if spaceship hits bottom of screen outside platform
        if spaceship_pos[1] + 15 >= height:
            wrong_area()
            gaming = False
            continue
        
        # Check polygon collision between spaceship and ground polygon
        if polygons_collide(spaceship_poly, ground_polygon):
          wrong_area()
            # Check vertical velocity for landing success or crash
            # if v1 <= 3.0:
            #     win()
            # else:
            #     lose()
        ...
        # Display info (velocity, altitude, fuel)
        info_y = 10
        info_spacing = 75
        ...
        # Draw off-screen indicators (your existing code)
...
# --- Your existing functions: start_screen(), leaderboard_screen(), wrong_area(), win(), lose(), cntdown() ---
# (Keep them unchanged)
